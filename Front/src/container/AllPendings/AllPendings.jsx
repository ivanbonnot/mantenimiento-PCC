...
const AllPendings = () => {
  ...
  const handleShareNote = async (estacion, title, note) => {
    try {
      if (navigator.share) {
        await navigator.share({
          title: 'Compartir Nota',
          text: `Lugar: ${estacion.toUpperCase()} \nTítulo: ${title}\nAclaración: ${note}`,
        });
      } else {
    ...
    <div className="app__bg app__notes-container">
      <div className="app__notes-wrapper">
        <div className="app__notes-read">
          ...
          {noteData.map(({ _id, idnota, estacion, title, note, fecha, creador }) => (
            <div className="app__notes-note" key={idnota}>
              <p className="title p__opensans">{estacion}</p>
              <p className="note p__opensans">Titulo: {title}</p>
              ...
              <div>
                ...
                {windowWidth < 768 && (
                  <button
                    ...
                    onClick={() => handleShareNote(estacion, title, note)}
                  >
